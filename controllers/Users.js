...
export const createUser = async (req, res) => {
  ...
  try {
    const hashPassword = await bcrypt.hash(password, 10); // Hash password menggunakan bcrypt
...
export const updateUser = async (req, res) => {
  ...
  try {
    ...
    if (password && password.trim() !== "") {
      hashPassword = await bcrypt.hash(password, 10); // Hash password menggunakan bcrypt
    }
...
export const getUsers = async (req, res) => {
  try {
    // Ambil daftar pengguna dari tabel User
...
// Fungsi untuk mendapatkan detail pengguna berdasarkan UUID
export const getUserById = async (req, res) => {
  try {
    // Cari pengguna berdasarkan UUID di tabel User
    ...
    // Mendapatkan informasi tambahan berdasarkan role dari tabel terkait
    switch (user.role.toLowerCase()) {
      case "petani":
